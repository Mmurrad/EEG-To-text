...
from deep_model_decoding import BrainTranslator, BrainTranslatorNaive, T5Translator, R1Translator
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'dev']:
            ...
            for input_embeddings, seq_len, input_masks, input_mask_invert, target_ids, target_mask, sentiment_labels in tqdm(dataloaders[phase]):
                ...
                # load in batch
                input_embeddings_batch = input_embeddings.to(device).bfloat16()
                input_masks_batch = input_masks.to(device).bfloat16()
                input_mask_invert_batch = input_mask_invert.to(device).bfloat16()
                ...
                #Newly added but the bottom one is old
                with torch.cuda.amp.autocast():
                    #seq2seqLMoutput = model(input_embeddings_batch.to(device),
                     #       input_masks_batch.to(device),
                      #      input_mask_invert_batch.to(device),
                       #     target_ids_batch.to(device))

                    #Following only comment for Llama, for others will be comment out
...
if __name__ == '__main__':
    ...
    elif model_name == 'LlamaTranslator':
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-2-7b-chat-hf")
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.pad_token_id = tokenizer.eos_token_id

    elif model_name == 'DeepSeekTranslator':
    ...
    model.to(device)
    # The following is comment only for Llama
    ...
    if model_name in ['BrainTranslator','BrainTranslatorNaive', 'PegasusTranslator', 'T5Translator', 'DeepSeekTranslator', 'R1Translator', 'LlamaTranslator']:
        for name, param in model.named_parameters():
    ...
    if skip_step_one:
        ...
        if model_name == 'DeepSeckTranslator':
            ...
            # Update training loop with autocast
            scaler = torch.cuda.amp.GradScaler()

           # for input_embeddings, ... in tqdm(dataloaders[phase]):
            #    with torch.cuda.amp.autocast(dtype=torch.bfloat16):
             #       seq2seqLMoutput = model(input_embeddings_batch, ...)
    ...
    # Check if DeepSeek model supports custom embeddings
    from transformers import AutoConfig

    config = AutoConfig.from_pretrained("deepseek-ai/deepseek-llm-7b-chat")
    print("Model max position embeddings:", config.max_position_embeddings)
    print("Hidden size:", config.hidden_size)
    ######################################################
    '''Following loop will continue for all model except Llama and step two trainig: update whole model for a few iterations'''
    ######################################################
    #for name, param in model.named_parameters():
     #   param.requires_grad = True


    ''' New set up optimizer and scheduler only for Llama'''
    ...
    '''Orginal set up optimizer and scheduler'''
